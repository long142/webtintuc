<aside id="sidebar" class="four column pull-right">
	<ul class="no-bullet">
		...
		<?php 
			$videoGet;
			$post_video = $posts->sortByDesc('created_at')->where('post_type','video')->take(1)->toArray(); 
			foreach ($post_video as $key => $value) {
				$videoGet = $value;
			}
		?>
		<li class="widget widget_video clearfix">
			...
			<div class="flex-video widescreen">
				<video src="{{ $videoGet['feture'] }}" alt="{{ $videoGet['title']}}" style="width: 100%" controls></video>
			</div>
		</li>
